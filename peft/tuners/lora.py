    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            if self.r > 0:
                ...
                for i in range(self.lora_num):
                    result = result + torch.unsqueeze(route_weight[:,:,i], -1) * getattr(self, f"lora_B{i}")(getattr(self, f"lora_A")(x)) * self.scaling
                    # result = result + torch.unsqueeze(route_weight[:,:,i], -1) * getattr(self, f"lora_B{i}")(self.lora_dropout(getattr(self, f"lora_A")(x))) * self.scaling
